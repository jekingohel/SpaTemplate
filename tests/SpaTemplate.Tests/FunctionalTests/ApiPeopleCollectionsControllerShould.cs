...
namespace SpaTemplate.Tests.FunctionalTests
{
	...
	public partial class ApiPeopleCollectionsControllerShould : IClassFixture<CustomWebApplicationFactory<Startup>>
	{
		...
		[Theory]
		[AutoMoqData]
		public async Task CreateCollection_AfterValidPostAsync(IEnumerable<StudentForManipulationDto> dtos)
		{
		...
		[Fact]
		public async Task ReturnsBadRequest_NoIdsAsync()
		{
		...
		[Theory]
		[AutoMoqData]
		public async Task ReturnsCollection_AfterValidGetAsync(IEnumerable<StudentForManipulationDto> dtos)
		{
			var post = await this.PostAsync(dtos).ConfigureAwait(false);
			Assert.Equal(HttpStatusCode.Created, post.StatusCode);
		...
		[Fact]
		public async Task ReturnsNotFound_IdsNotExistAsync()
		{
	...
	public partial class ApiPeopleCollectionsControllerShould
	{
		private Task<HttpResponseMessage> GetAsync(IEnumerable<Guid> ids = null) =>
			this.client.GetAsync(Api.StudentCollectionsIds.ToApiUrl(ids));

		private Task<HttpResponseMessage> PostAsync(IEnumerable<StudentForManipulationDto> dtos) =>
			this.client.PostAsync(Api.StudentCollections, dtos.Content(MediaType.InputFormatterJson));
	}
}