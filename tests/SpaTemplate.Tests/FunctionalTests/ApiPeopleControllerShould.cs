...
namespace SpaTemplate.Tests.FunctionalTests
{
	...
	public partial class ApiPeopleControllerShould : IClassFixture<CustomWebApplicationFactory<Startup>>
	{
		...
		[Fact]
		public async Task A1_GetPaginationNextPageNotNullAsync()
		{
		...
		[Fact]
		public async Task A1_GetPaginationPreviousPageNotNullAsync()
		{
		...
		[Fact]
		public async Task A1_GetReturnsCollectionAsync()
		{
		...
		public async Task A1_GetReturnsCollection_ValidMethodAndRelInsideLinksAsync(string method, string rel, int number)
		{
		...
		public async Task A1_GetReturnsCollection_ValidMethodAndRelOutsideLinksAsync(string method, string rel, int number)
		{
			var get = await this.GetHateoasReadAsStringAsync().ConfigureAwait(false);
		...
		[Fact]
		public async Task A1_GetReturnsNotFound_EntityNotExistAsync()
		{
		...
		public async Task A1_GetReturnsNotNullHrefAsync(string fields, string rel)
		{
		...
		[Fact]
		public async Task A1_GetReturnsOk_ValidGetAsync()
		{
			var get = await this.GetAsync().ConfigureAwait(false);
		...
		[Theory]
		[AutoMoqData]
		public async Task A2_PostReturnsCreatedAsync(StudentForUpdateDto studentForUpdateDto)
		{
		...
		[Fact]
		public async Task A3_PatchReturnsNotFound_EntityForPatchNotExistAsync()
		{
		...
		[Fact]
		public async Task A5_DeleteReturnsNotFoundEntityAsync()
		{
		...
		public async Task B1_GetReturnsCorrectStatusCodeAsync(
			string field,
		...
		[Theory]
		[AutoMoqData]
		public async Task B3_PatchReturnsNoContent_ValidPatchAsync(StudentForUpdateDto studentForUpdateDto)
		{
		...
		[Theory]
		[AutoMoqData]
		public async Task B3_PatchReturnsUnprocessableEntity_EqualFieldsAsync(StudentForUpdateDto studentForUpdateDto)
		{
			var post = await this.PostReadAsStringAsync<StudentDto>(studentForUpdateDto).ConfigureAwait(false);
		...
		[Theory]
		[AutoMoqData]
		public async Task C5_DeleteRemoveStudentAsync(StudentForCreationDto studentForCreationDto)
		{
			var post = await this.PostReadAsStringAsync<StudentDto>(studentForCreationDto).ConfigureAwait(false);
	...
	public partial class ApiPeopleControllerShould
	{
		...
		private Task<HttpResponseMessage> GetAsync(Guid id, string fields = "") =>
			this.client.GetAsync(Api.Student.ToApiUrl(id, fields));

		private Task<HttpResponseMessage> GetAsync(string fields = "") =>
			this.client.GetAsync(Api.People.ToApiUrl(fields));
		...
		private Task<HttpResponseMessage> PostAsync(StudentForManipulationDto studentDto) =>
			this.client.PostAsync(Api.People, studentDto.Content(MediaType.PatchFormatterJson));
		...
		private Task<HttpResponseMessage> PatchAsync(Guid studentId, JsonPatchDocument<StudentDto> patchDoc) =>
			this.client.PatchAsync(Api.Student.ToApiUrl(studentId), patchDoc.Content(MediaType.PatchFormatterJson));

		// <--------------------------------------------------------PATCH-------------------------------------------------------->

		// <--------------------------------------------------------DELETE-------------------------------------------------------->
		private Task<HttpResponseMessage> DeleteAsync(Guid studentId) =>
			this.client.DeleteAsync(Api.Student.ToApiUrl(studentId));
