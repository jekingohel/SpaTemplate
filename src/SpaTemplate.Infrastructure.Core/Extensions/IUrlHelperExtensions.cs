...
namespace SpaTemplate.Infrastructure.Core
{
	public static class UrlHelperExtensions
	{
		...
		private static string CreateResourceUri(this IUrlHelper urlHelper, string routeName,
			IParameters parameters, ResourceUriType type)
		{
			switch (type)
			{
		...
		private static string PreviousPage<TEntity>(this IUrlHelper urlHelper, string routeName,
			IParameters parameter, IPagedList<TEntity> entities) where TEntity : BaseEntity =>
			entities.HasPrevious
				? urlHelper.CreateResourceUri(routeName, parameter,
					ResourceUriType.PreviousPage)
				: null;

		private static string NextPage<TEntity>(this IUrlHelper urlHelper, string routeName,
			IParameters resourceParameters, IPagedList<TEntity> entities) where TEntity : BaseEntity =>
			entities.HasNext
				? urlHelper.CreateResourceUri(routeName, resourceParameters,
					ResourceUriType.NextPage)
				: null;


		public static IEnumerable<ILinkDto> CreateLinks<TEntity>(this IUrlHelper urlHelper,
			string routeName, IParameters parameter, IPagedList<TEntity> pagedList)
		...
		public static HateoasPagination CreateHateoasPagination<T>(this IUrlHelper urlHelper,
			string routeName, IPagedList<T> pagedList, IParameters parameter)
